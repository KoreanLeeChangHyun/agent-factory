...
- 모르면 **다시 묻는다** (MUST)
- 헷갈리면 **다시 묻는다** (MUST)
- 사용자 명시 동의 없이 **자동 강제 정책·가드·FSM 룰·status 강제 전이·Conveyor 자동 회귀 등 안전장치 도입 절대 금지** (MUST NOT) — "사용자가 실수로 X 할 수 있으니 자동 차단" 같은 추측 기반 안전장치 삽입 금지
...
- 상태 보고 전 `flow-conveyor board` / `flow-conveyor list` 조회 필수 — 기억 기반 보고 금지
- WorkRequest 관련 발화(WorkRequest 번호 언급 / WorkRequest 생성·수정·이동·삭제 / WorkRequest 상태 보고 / "이거/저거/그거" 같은 지시어로 WorkRequest 가리킴) 시 반드시 `flow-conveyor show <WR-NNN>` 또는 `flow-conveyor board` 등으로 **현재 상태를 먼저 조회한 뒤 응답** (MUST). 기억·auto memory·jsonl 헤더(`_meta.work_request`)·세션 파일명·로그·이전 응답 결과 등 **Conveyor 외 보조 단서로 WorkRequest 식별·상태 단정 금지** (MUST NOT) — Conveyor가 single source of truth. Conveyor는 다른 세션·UI(board DnD)에서도 변경되므로 stale 보조 정보로 사용자 혼선 유발
...
- 어시스턴트→사용자 출력은 WorkRequest 용어만 사용한다 (MUST). 작업 진행 의향을 물을 때는 "WorkRequest 생성 진행할까요" / "룰 추가할까요" / "메모리 등록할까요" 등 구체 작업 어휘로 대체. WorkRequest 생성 의도 발화는 grill-me Ouroboros 5단계 루프 (DRAFT → CLARIFY → CRITIQUE → REWRITE → ACCEPT) 진입 트리거로 처리
- 코드 블록(fenced code block, ```...```)은 **실제 소스 코드를 보여줄 때만** 사용 (MUST). ASCII 다이어그램·디렉터리 트리·카드 구조도·레이아웃 스케치는 코드 블록 회피 (일반 텍스트·표·불릿으로 표현). frontmatter/JSON/YAML 등 실제 파일 내용 인용은 OK. 명령어 (`flow-conveyor create` 등) 는 인라인 backtick OK, fenced block 은 다중 줄 코드일 때만

## 사용자 응대 캐논 (MUST)

본 섹션은 어시스턴트가 사용자와 상호작용할 때 따라야 하는 통합 규약. 신규 세션 시작·WorkRequest 생성·진단 발화·결정 스타일 전반에 적용.
...
- **단정 전 데이터 조회 필수**: Conveyor/runs/세션/git/.pyc 조회 후 발언. 단정 강요 금지
- **표현 명확성**: 비전문 표현으로 사용자 혼선 유발 회피

### 진단 우선순위 (도구 우선, Single Source of Truth)
Conveyor/도구 우선 진단. 메모리·세션 파일명·jsonl 헤더·코드 차단 주석·PID·lstart = 모두 보조 단서.

| 발화 종류 | 직접 조회 도구 |
|-----------|---------------|
| WorkRequest 상태/번호/지시어 | `flow-conveyor show <WR-NNN>` / `list --status N` / `board` |
...
**Verifying 도달 ≠ 작업 완료**: driver finalize 가드 사각지대 (work/+report만 검증) 존재. 워크트리 commits ahead 동시 검증 의무.
...
### 우선순위
- **워크플로우 + 워크트리 + Conveyor + git 4축 유기 작동 = 최우선 과제**. 신규 기능보다 4축 정합성 우선
...
### 카드 UI 가시성 양 축
- **컬럼 의미 가시성**: 각 컬럼 (특히 Complete) 이 정확히 그 상태의 WorkRequest만 담아야 시야 유지. 잔재 섞이면 가시성 붕괴 → "진짜 완료만 Complete" 룰의 근거
- **카드 자체 가시성**: Verifying 카드의 verdict / 워크트리 상태 / 검증 이력 / Complete 처리 진입로 (버튼/메뉴/DnD) 모두 카드만 봐도 즉시 파악 가능해야

### Complete 처리 정책
- "완료는 진짜 완료된 WorkRequest만" — 사용자 테스트 통과 + 사용자 명시 동의 후. 옛 잔재 정리 시 Complete 옵션 제시 금지, 삭제 (`/wf -c`) 또는 Draft 유지만 제안
- 어시스턴트가 "코드 구현됐으니 Complete" 추론 금지

### Agent Role 어휘 분리 (v2 driver 모델)

| 어휘 | 의미 | 실체 |
|------|------|------|
| **driver** | v2 워크플로우 엔진 본체 | `.agent-factory/bin/flow-wf` → `engine/v2/driver.py`. 룰베이스 결정론 — 14룰 평가 + auto_commit + Conveyor 전이 + FSM 전이. LLM 호출 X |
| **claude -p subprocess** | Step 별 LLM 작업자 | INIT/PLAN/WORK/VALIDATE/REPORT 각 Step 마다 driver 가 spawn 하는 별도 프로세스. 산출물 .md 본문만 작성 |
| **메인 세션** | 사용자 대화 전용 | Claude Code 본체. WorkRequest 관리·상태 확인·결과 리뷰 등 조율. **오케스트레이터 아님** — 워크플로우 진행은 driver 가 담당 |
...
## 메인 세션 제약
- 서브에이전트(Agent 도구) 사용 금지 (MUST NOT) — 시간이 오래 걸리므로 WorkRequest 생성 후 워크플로우로 처리
...
## WorkRequest 운영
- WorkRequest 과분리 금지 — 관련 항목은 스프린트/복잡도 단위로 묶기
