...
| 플래그 | 용도 | 예시 |
|--------|------|------|
| `/wf -o` | 새 티켓 생성 (채번+용도선택만, To Do/Open 상태 선택) | `/wf -o` |
| `/wf -o N` | 기존 티켓 열람 (To Do는 유지, 그 외는 Open 전이) | `/wf -o 3` |
| `/wf -e` | 새 티켓 생성 + 프롬프트 편집 (To Do/Open 상태 선택) | `/wf -e` |
| `/wf -e N` | 기존 티켓 편집 (To Do는 Open으로 자동 승격) | `/wf -e 3` |
| `/wf -oe` | 새 티켓 생성 + 편집 (`-o -e` 단축) | `/wf -oe` |
| `/wf -oe N` | 기존 티켓 Open + 편집 (`-o -e` 단축) | `/wf -oe 3` |
| `/wf -s N` | 티켓 제출 및 워크플로우 실행 (To Do 상태면 에러) | `/wf -s 3` |
...
> **칸반 전이**: (없음) -> **To Do** 또는 **Open** (사용자 선택)

#### 1-0. 초기 상태 선택 (To Do vs Open)

사용자 발화에서 상태 신호를 감지하여 기본값을 추천하되, 반드시 번호 메뉴로 확인합니다:

| 발화 패턴 | 추천 상태 |
|----------|---------|
| "티켓 생성", "나중에", "언젠가", "백로그" | To Do |
| "지금", "바로", "이번에", "집중" | Open |
| 무맥락 | (추천 없음) |

번호 메뉴 출력:
```
`[T-NNN]` : `[WF -o]` 이 티켓을 어디에 둘까요?

`1.` To Do -- 미래에 할 백로그·티켓 생성 공간
`2.` Open -- 지금 집중해야 하는 임박 작업
```

사용자 선택에 따라 `--status todo` 또는 `--status open`을 결정합니다. 사용자가 발화에서 이미 상태를 명시("Open으로 만들어줘" 등)한 경우에만 질의를 생략합니다.
...
```bash
flow-kanban create "" --command init --status todo
# 또는
flow-kanban create "" --command init --status open
```

stdout에서 T-NNN을 파싱하고 채번 결과를 출력합니다:
```
`[T-NNN]` : `[WF -o]` 티켓 T-NNN을 생성했습니다. (상태: <선택된 상태>)
```
...
### Step 1-B-o. 번호 있음: 기존 티켓 내용 표시 (편집 루프 미진입)

편집 루프에 진입하지 않습니다. To Do 상태 티켓은 상태를 유지한 채 내용만 표시합니다.
...
**To Do 상태 유지 시:**
```
`[T-NNN]` : `[WF -o]` T-NNN 티켓 내용을 표시했습니다. (현재 상태: To Do)

## 후속 커맨드 안내

| 목적 | 커맨드 | 설명 |
|------|--------|------|
| 편집 | `/wf -e N` | Open으로 승격 후 프롬프트 편집 |
| 종료 | `/wf -d N` | 티켓을 Done 상태로 종료합니다 |
```
...
stdout에서 T-NNN을 파싱하고 채번 결과를 출력합니다:
```
`[T-NNN]` : `[WF -e]` 티켓 T-NNN을 생성했습니다. (상태: <선택된 상태>)
```
...
> 참고: 선택된 상태가 `todo`이면 `/wf -s N` 실행 전에 `/wf -e N`으로 Open 승격이 필요합니다. v2 driver 는 단일 command 만 지원합니다 (체인 `research>implement>review` 는 Phase 4 후속 트랙, SPEC.md §14 참조).
```

### Step 1-B-e. 번호 있음: 기존 티켓 편집 루프

> **칸반 전이**: To Do -> **Open** (승격) / Done -> **Open** (복원) / Review/In Progress -> **Open** (자동 복귀) / Open -> Open (유지)
...
#### 1-B-2. To Do / Review / In Progress 상태 자동 Open 복귀

현재 상태가 `To Do`, `In Progress`, 또는 `Review`인 경우, 사용자 확인 없이 즉시 Open으로 전이합니다:
```bash
flow-kanban move T-NNN open
```

- `To Do` -> `Open`: 승격 (편집 의도이므로 즉시 집중 대상으로 전환)
- `In Progress` / `Review` -> `Open`: 자동 복귀
- `Open`: 상태 변경 없이 즉시 편집 루프로 진입
...
- "0. 완료" 선택 시 constraints/criteria 필수 검증 후:
  1. `flow-kanban create "<새 제목>" --command <command값> --status open` 로 새 티켓 T-MMM 생성 (파생 티켓은 즉시 집중 대상이므로 기본 Open)
...
```
`[T-NNN]` : `[WF -s]` T-NNN은 To Do 상태입니다. 먼저 `/wf -e N`으로 Open 승격 후 다시 제출하세요.
```
...
```
`[T-NNN]` : `[WF -s]` v2 driver 는 단일 command 만 지원합니다 (체인 `research>implement>review` 미지원). `/wf -e N` 으로 `<command>` 값을 단일 command 로 갱신 후 다시 제출하세요.
```
...
```
`[T-NNN]` : `[WF -s]` T-NNN 티켓을 <command> 워크플로우로 실행합니다 (v2 driver background).
...
>
> **Review 단계 의미**: 워크플로우 완료 후 보고서 검토 + Review 카드 4행 토글로 feature 브랜치를 활성화하여 사용자가 직접 테스트 + 사용자 리뷰. (카드 토글 ON: feature 브랜치로 git switch, OFF: develop으로 복귀)
>
> **Done 의미**: 머지 + 사용자 직접 테스트 통과 후의 진짜 종결. 코드 변경 + 사용자 검증 양쪽이 끝난 상태만 Done으로 처리한다.
...
| 그 외 (To Do, Open, In Progress 등) | 3-B. 즉시 Done 처리 (기존 로직) |
...
- 새 review 티켓을 생성하고 원본 티켓과 link합니다:
  1. `flow-kanban create "T-NNN 상세 리뷰" --command review --status open` 로 새 티켓 T-MMM 생성 (상세 리뷰는 즉시 집중 대상)
...
|--------|------------|------------|---------|
| `-o` (번호 없음) | (없음) | To Do 또는 Open (사용자 선택) | 번호 메뉴 질의 후 `flow-kanban create "" --command init --status <todo|open>` + `flow-kanban update-prompt --skip-validation` (채번+용도선택만, 편집 루프 미진입) |
| `-e` (번호 없음) | (없음) | To Do 또는 Open (사용자 선택) | 번호 메뉴 질의 후 `flow-kanban create "" --command init --status <todo|open>` + `flow-kanban update-prompt` + 편집 루프 (`-oe`도 동일 동작) |
| `-o N` | To Do | To Do (유지) | -- (내용 표시만, 편집 루프 미진입. 상태 유지) |
...
| `-e N` | To Do | Open (승격) | `flow-kanban move T-NNN open` + 편집 루프 (편집 의도이므로 즉시 집중 대상으로 승격) |
...
| 단계 | 플래그 | 티켓 상태 | 수행 명령 |
|------|--------|----------|---------|
| 티켓 생성 직후 | `-o` 또는 `-e`(또는 `-oe`) | `<prompt>` 미존재, `<result />` self-closing | `flow-kanban create "" --command init --status <todo|open>` (`--status`는 사용자 선택 필수) |
...
| 후속 작업 필요 | `-e NNN` (실행 완료 후 편집) | 새 티켓 생성 + link | `flow-kanban create --status open` + `flow-kanban update-prompt T-MMM` + `flow-kanban link T-MMM --derived-from T-NNN` |
...
12. **To Do 상태 정책**: `-o`/`-e` 생성 시 반드시 `--status todo` 또는 `--status open`을 지정해야 합니다 (T-385 이후 `--status` 필수화). 사용자 발화에 상태가 명시되지 않았으면 번호 메뉴로 질의합니다 (AskUserQuestion 미사용, 텍스트 번호 메뉴 사용). To Do 상태 티켓에 `-s`를 시도하면 에러로 종료되므로, 먼저 `-e N`으로 Open 승격이 필요합니다. `-o N`은 To Do 상태를 유지한 채 내용만 표시합니다. `-e N`은 편집 의도이므로 To Do를 Open으로 자동 승격합니다
...
|-----------------|-----------|----------|------|
| "티켓만 열어", "채번해줘", "티켓 하나 만들어" | `-o` | (질의) | 생성만 요청, 편집 의도 없음 |
| "이거 티켓 생성해줘", "나중에 할 거야", "언젠가 할 일", "백로그에 넣어" | `-o` 또는 `-e` | To Do | 미래 작업 의도 명시 |
| "지금 바로 할래", "이번에 집중할 거야", "당장 진행" | `-o` 또는 `-e` | Open | 즉시 집중 대상 의도 명시 |
| "티켓 만들고 편집할게", "프롬프트 작성하자", "편집할게" | `-e` | (질의) | 생성 + 편집 의도 명시 (`-oe`는 `-e`의 단축 별칭) |
| "기존 티켓 열어", "3번 티켓 보여줘" | `-o N` | 상태 유지 | 열람만 요청, 편집 의도 없음 |
| "3번 티켓 수정할게", "기존 티켓 편집하자" | `-e N` | To Do -> Open 승격 | 편집 의도 명시 (`-oe N`도 동일 동작) |
...
> (4) **상태 선택**: "티켓 생성/나중에/언젠가/백로그" -> To Do 추천, "지금/바로/이번에/집중" -> Open 추천, 무맥락이면 번호 메뉴로 질의
