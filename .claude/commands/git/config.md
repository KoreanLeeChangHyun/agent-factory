---
description: .agent-factory/.settings에서 Git 설정을 읽어 git config를 자동 설정합니다.
...
`.agent-factory/engine/git/git_config.py` - 옵션: `[--global|--local]`
...
- `.agent-factory/.env` 파일에서 환경변수 로드
...
| 오류 상황 | 대응 |
|----------|------|
| `.agent-factory/.env` 파일 없음 | 에러 메시지 출력 후 중단 |
