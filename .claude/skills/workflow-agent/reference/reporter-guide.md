...
> **책임 경계**: history.md 갱신, status.json 완료 처리(REPORT->DONE), 사용량 확정, 레지스트리 해제(directory-scan 기반 자동 해제), DONE 배너는 오케스트레이터가 flow-finish로 수행합니다. reporter는 보고서 작성에만 집중합니다.

> **Slack 완료 알림**: reporter는 Slack 호출을 수행하지 않습니다. Slack 완료 알림은 `flow-claude end <registryKey>`에서 자동 전송됩니다.
...
- **출력 허용**: 보고서 파일 경로, 반환값 (1줄 규격), 에러 메시지
...
```bash
flow-update context <registryKey> reporter
```
...
> 상세 정보는 보고서 파일(.agent-factory/runs/)에 저장되어 있으므로 반환에 포함할 필요가 없습니다.
...
> **금지 항목**: 보고서 내용 요약, 변경 파일 목록, "다음 단계" 안내, 실행 로그, 경로, 메타정보(N개) 등을 반환에 포함하지 않습니다. 이러한 정보는 보고서 파일에만 기록합니다.
...
| 다이어그램 | 워크플로우, 구조 시각화 | design-mermaid-diagrams 스킬 연동 |
...
- 산출물 경로(work/WXX-*.md, report.md 등 .agent-factory/runs/ 하위)는 백틱 유지, 링크 미적용

**이중 접두사 버그 주의**: 링크 경로에 workDir 접두사를 중복 부여하지 않습니다. 보고서가 `.agent-factory/runs/<id>/` 하위에 위치하므로, 파일 경로는 프로젝트 루트 기준 상대 경로를 그대로 사용합니다. `../workflow/../workflow/` 같은 이중 접두사가 발생하면 링크가 깨집니다.
...
> **Note**: reporter 반환 후, 오케스트레이터가 flow-finish로 status.json 완료 처리 및 레지스트리 해제를 수행합니다.
