...
- [ ] 배너 출력 스크립트: `.agent-factory/engine/banners/`
- [ ] 워크플로우 흐름 제어 스크립트: `.agent-factory/engine/flow/`
- [ ] 가드/보안 스크립트: `.agent-factory/engine/guards/`
- [ ] 동기화 스크립트: `.agent-factory/engine/sync/`
- [ ] Hook 디스패처: `.agent-factory/hooks/` (실제 로직은 `scripts/`에 분리)

### 현재 등록된 alias 목록

| alias | 스크립트 경로 | 용도 |
|-------|-------------|------|
| `flow-claude` | `.agent-factory/engine/banners/flow_claude_banner.sh` | 워크플로우 시작/종료 배너 |
| `flow-step` | `.agent-factory/engine/banners/flow_step_banner.sh` | 스텝 시작/종료 배너 |
| `flow-phase` | `.agent-factory/engine/banners/flow_phase_banner.sh` | WORK 페이즈 배너 |
| `flow-init` | `python3 .agent-factory/engine/flow/initialization.py` | 워크플로우 초기화 |
| `flow-finish` | `python3 .agent-factory/engine/flow/finalization.py` | 워크플로우 마무리 처리 |
| `flow-reload` | `python3 .agent-factory/engine/flow/reload_prompt.py` | 프롬프트 리로드 |
| `flow-update` | `python3 .agent-factory/engine/flow/update_state.py` | 워크플로우 상태 관리 |
| `flow-skillmap` | `python3 .agent-factory/engine/flow/skill_mapper.py` | 태스크별 스킬 매핑 생성 |
| `flow-validate` | `python3 .agent-factory/engine/flow/plan_validator.py` | 계획서 유효성 검증 |
| `flow-validate-p` | `python3 .agent-factory/engine/flow/prompt_validator.py` | 프롬프트 유효성 검증 |
| `flow-recommend` | `python3 .agent-factory/engine/flow/skill_recommender.py` | 스킬 자동 추천 |
| `flow-gc` | `python3 .agent-factory/engine/flow/garbage_collect.py` | 좀비 워크플로우 정리 |
| `flow-kanban` | `python3 .agent-factory/engine/flow/kanban.py` | 칸반 보드 관리 |
| `flow-merge` | `python3 .agent-factory/engine/flow/merge_pipeline.py` | PR 기반 머지 파이프라인 |
| `flow-launcher` | `.agent-factory/bin/flow-launcher` | HTTP API 기반 워크플로우 세션 실행 |
| `flow-history` | `python3 .agent-factory/engine/sync/history_sync.py` | 히스토리 동기화 |
| `flow-catalog` | `python3 .agent-factory/engine/sync/catalog_sync.py` | 스킬 카탈로그 재생성 |
| `flow-gitconfig` | `python3 .agent-factory/engine/git/git_config.py` | Git config 설정 |
| `flow-detect` | `python3 .agent-factory/engine/flow/project_skill_detector.py` | 프로젝트 스킬 감지 |

> alias 추가 시 `build.sh`의 `setup_shell_aliases()` 함수 내 `.claude.aliases` heredoc에 항목을 추가한다.

**참고**: `build.sh` (alias 등록 위치), `.agent-factory/engine/` (실제 로직 스크립트 디렉터리), [reference/hooks-guide.md](reference/hooks-guide.md) (Hook 이벤트와 스크립트 연동 방법)
...
if cwd and current_session:
    workflow_dir = os.path.join(cwd, ".agent-factory", "workflow")
    try:
        for date_dir in sorted(os.listdir(workflow_dir), reverse=True):
            ...
            for name_dir in os.listdir(date_path):
                ...
                for cmd_dir in os.listdir(name_path):
                    work_dir = os.path.join(".agent-factory", "workflow", date_dir, name_dir, cmd_dir)
