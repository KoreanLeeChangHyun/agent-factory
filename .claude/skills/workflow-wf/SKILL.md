...
1. **계층적 정보 배치**: 요약 -> 상세 -> 근거 순으로 정보를 배치하여 독자가 깊이를 선택할 수 있게 한다
2. **비교 가능한 형식**: 동일 기준으로 대상을 비교하는 테이블/매트릭스를 활용하여 객관적 판단을 돕는다
3. **근거 추적 가능성**: 모든 핵심 주장에 출처와 등급(S/A/B/C/D)을 병기하여 신뢰도를 투명하게 한다

### 연구 절차

1. **주제 파악 및 범위 정의**
   - 조사 유형 분류: 기술 조사(특정 기술 심층) / 개념 연구(이론·패턴) / 비교 분석(복수 대안 평가) / 내부 자산 분석(코드베이스·DB 등)
   - 유형별 접근: 기술 조사는 공식 문서 우선, 비교 분석은 평가 축 선정 우선, 내부 분석은 코드 탐색 우선
   - 조사 깊이, 범위, 시간 범위 정의 및 비교 대상 확인 (해당 시)

2. **정보 수집**
   - WebSearch: 핵심 키워드 + 연도 조합으로 최신 정보 검색, 공식 문서 URL 식별 후 WebFetch로 상세 확인
   - WebFetch: 공식 문서·RFC·표준 규격(S등급) 우선 수집, 기술 블로그는 교차 검증용 보조
   - Grep/Glob/Read: 코드베이스 현황 스냅샷 수집 -- 관련 패턴 사용 현황, 의존성 버전, 설정 파일 확인

3. **분석 및 정리**
   - 핵심 개념 추출 및 장단점 분석
   - 비교 분석 시: 최소 3개 비교 축을 포함한 다차원 비교 매트릭스 작성, 각 축마다 트레이드오프 명시
   - 실제 적용 가능성 평가 및 주의사항/제한사항 병기
   - 각 주장에 반론 또는 제한 조건을 함께 기술하여 균형 잡힌 분석 유지

4. **리포트 작성**
   - 근거 기반 결론 도출: 주장-근거-출처 3단 구조로 핵심 결론을 작성
   - 구조화된 문서 생성, 출처 명시
...
조사 완료 전 워커가 자체 검증할 항목. 모든 항목을 만족해야 보고서를 제출한다.

- [ ] 핵심 결론마다 S-A 등급 출처 최소 1개 첨부
- [ ] 비교 분석 시 최소 3개 비교 축 포함
- [ ] 각 주장에 반론/제한사항 병기
- [ ] 코드베이스 현황과 외부 정보 교차 대조
- [ ] 실행 가능한 권고사항 포함
...
- 인라인 표기: `[등급] 출처명 (URL, YYYY)` -- 예: `[S] React 공식 문서 (https://react.dev, 2026)`
- 참고 자료 섹션: 리포트 말미에 등급별로 그룹화하여 나열, S/A 등급을 상단에 배치
...
| 스킬 | 용도 | 로드 시점 |
|------|------|----------|
| research-general | 연구/조사 워크플로우 상세 정의, 리포트 템플릿 | 모든 research 실행 시 기본 로드 |
| research-integrated | 웹+코드 통합 조사 | 웹 정보와 코드베이스를 교차 분석해야 할 때 로드 |
| analyze-srs | 요구사항 분석 절차 | 분석 유형 매칭 시 |
| analyze-codebase | 코드베이스 분석 절차 | 분석 유형 매칭 시 |
| analyze-database | 데이터베이스 분석 절차 | 분석 유형 매칭 시 |
| analyze-data | 데이터 분석 절차 | 분석 유형 매칭 시 |
