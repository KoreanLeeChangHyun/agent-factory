...
T-500: separated from the conveyor.py ` handle conveyor submit` body. handler input validation +
...
Terms and Conditions:
  - `spawn_production_line(work_request, command) -> dict`:
      ...
      * `flow-wf submit <work_request>` Popen.
      * LAUNCH PENDING + LAUNCH STARTED Launch.
      * thread spawn + ` LAUNCH READER THREADS` registered.
      * Return: `{ok, status, work_request, command, submission at, session id}` (Property) /
              `{ok: False, error kind, message}` (fail).
  - `_production_line_reader_loop(proc, work_request, command, submitted_at)`:
...
Original Position: handlers/conveyor.py:194-244 + 510-610 (T-500 before).
"""
...
# v1 ` launch reader loop` (conveyor.py) share the same set — in conveyor.py
...
def _emit_launch_event_safe(event: str, work_request: str, **kwargs: object) -> None:
    """`engine.apps.board_api.conveyor._emit_launch_event` lazy import wrapper.

    Module top-level import conveyor
    import module top. emits itself absorbs broadcasting failure.
    """
    try:
        from engine.apps.board_api.conveyor import _emit_launch_event
    except ImportError:  # Defending — this import failure is an environmental problem
        return
    _emit_launch_event(event, work_request, **kwargs)


def spawn_production_line(work_request: str, command: str) -> dict:
    """'flow-wf submission <work_request>` subprocess spawn + LAUNCH PENDING/STARTED launch.

    conveyor handler is only pre-programmed for work_request / command validation
    ...
    Returns:
        success: `{"ok": true, "status": "starting", "work_request", "command",
                "submitted_at": <iso>, "session_id": <wf-WR-NNN-key>}`
    ...
    production_line_session_id = f'wf-{work_request}-{registry_key}'
    ...
    reader = threading.Thread(
        ...
        args=(proc, work_request, command, submitted_at),
        name=f'production-line-reader-{work_request}',
        daemon=True,
    )
...
def _production_line_reader_loop(
    ...
    v1 ` launch reader loop` (conveyor.py) and difference:
