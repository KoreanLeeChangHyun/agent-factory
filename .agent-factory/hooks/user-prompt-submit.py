...
플래그 제어:
  HOOK_USER_PROMPT_KANBAN=true/false (.agent-factory/.settings)
  기본값은 비활성(flags에 미정의 시 is_enabled가 True를 반환하지만,
  dispatch() 내부의 is_enabled 기본값 동작을 따름)

디버그 로그:
  HOOK_USER_PROMPT_DEBUG=true 설정 시 .agent-factory/runs/.user_prompt_hook.log 에 append.
  off-by-default.
"""
...
def _is_main_session(stdin_data: dict) -> bool:
    ...
    2. stdin JSON 의 "cwd" 필드:
       - ".agent-factory/worktrees/" 를 포함하면 → 워크트리 세션 → False
    ...
    worktree_marker = os.path.join('.agent-factory', 'worktrees') + os.sep
    ...
    if transcript_path:
        ...
        # 워크플로우 runs 경로: .agent-factory/runs/YYYYMMDD-HHMMSS/티켓명/implement/.context.json
    ...
    # --- 4. 보수적 기본값: cwd가 .agent-factory/runs/ 아래이면 워크플로우 세션 ---
