...
Dispatches subagent-stop hooks based on HOOK_* flags in .agent-factory/.settings.
Replaces individual wrapper scripts in .agent-factory/hooks/subagent-stop/.
...
def _resolve_fail_record_bin() -> str | None:
    """`.agent-factory/bin/flow-fail-record` 절대 경로를 반환한다.
    ...
    candidate = os.path.join(
        project_root, ".agent-factory", "bin", "flow-fail-record"
    )
