# .agent-factory 리네이밍 인벤토리 — 2026-04-22

본 문서는 `.claude.workflow` → `.agent-factory` 전면 리네이밍 작업의 인벤토리 및 확정 매핑이다. Phase 1~4 작업 중 참조 기준이 된다.
...
| 현재 | 변경 | 비고 |
|---|---|---|
| `.claude.workflow/` | `.agent-factory/` | 확정 (사용자 결정) |
...
| **P1** | 루트 `.claude.workflow/` → `.agent-factory/` **git mv만** + `.claude/settings.json` + `init-claude-workflow.sh` + `.claude/rules/*`에 포함된 리터럴 치환. 내부 디렉터리 구조는 미변경 | 1 |
...
- Phase 1에서 루트만 rename하면 `.agent-factory/scripts/claude_edit.py`로 살아남음. Phase 2에서 내부도 rename되면서 `.agent-factory/engine/claude_edit.py`로 최종 이동.
...
- [x] 신규 브랜치 `refactor/agent-factory-rename` 생성
- [x] 본 인벤토리 문서 작성
- [ ] Phase 1 착수
