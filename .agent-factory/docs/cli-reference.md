...
- **alias**: `flow-claude`
- **스크립트**: `.agent-factory/engine/banners/flow_claude_banner.sh`
...
- **alias**: `flow-step`
- **스크립트**: `.agent-factory/engine/banners/flow_step_banner.sh`
...
- **alias**: `flow-phase`
- **스크립트**: `.agent-factory/engine/banners/flow_phase_banner.sh`
...
- **alias**: `flow-init`
- **스크립트**: `.agent-factory/engine/flow/initialization.py`
...
- `workDir`: `.agent-factory/runs/<YYYYMMDD-HHMMSS>/` 직속 디렉터리 구조
...
- **alias**: `flow-finish`
- **스크립트**: `.agent-factory/engine/flow/finalization.py`
...
- **alias**: `flow-reload`
- **스크립트**: `.agent-factory/engine/flow/reload_prompt.py`
...
| 인자 | 필수 | 타입 | 설명 |
|------|------|------|------|
| `workDir` | 필수 | string | 작업 디렉터리 상대 경로 (예: `.agent-factory/runs/20260325-004729/`) |

#### 환경변수

| 변수 | 설명 |
|------|------|
| `TICKET_NUMBER` | 티켓 번호 (T-NNN 또는 NNN). 미지정 시 .context.json 또는 .agent-factory/tickets/active/ 스캔으로 자동 탐색 |
...
```bash
flow-reload .agent-factory/runs/20260325-004729
```
...
- **alias**: `flow-update`
- **스크립트**: `.agent-factory/engine/flow/update_state.py`
...
| `env` | `flow-update env <registryKey> set\|unset <KEY> [VALUE]` | .agent-factory/.env 환경변수 관리 |
...
- **alias**: `flow-skillmap`
- **스크립트**: `.agent-factory/engine/flow/skill_mapper.py`
...
- **alias**: `flow-skill`
- **스크립트**: `.agent-factory/engine/flow/skill_state_manager.py`
...
- **alias**: `flow-validate`
- **스크립트**: `.agent-factory/engine/flow/plan_validator.py`
...
```bash
flow-validate .agent-factory/runs/20260325-004729/plan.md
```
...
- **스크립트**: `.agent-factory/engine/flow/prompt_validator.py`
...
| 인자 | 필수 | 타입 | 설명 |
|------|------|------|------|
| `prompt_file_path` | 필수 | string | 티켓 XML 파일 경로 (.agent-factory/tickets/active/T-NNN.xml) |
...
```bash
flow-validate-p .agent-factory/tickets/active/T-169.xml
```
...
- **스크립트**: `.agent-factory/engine/flow/skill_recommender.py`
...
- **alias**: `flow-gc`
- **스크립트**: `.agent-factory/engine/flow/garbage_collect.py`
...
- `.agent-factory/runs/` 하위에서 TTL 만료 + 미완료(DONE/FAILED/STALE 미해당) status.json을 STALE로 전환
...
- **alias**: `flow-kanban`
- **스크립트**: `.agent-factory/engine/flow/kanban.py`
- **설명**: 칸반 보드 상태 관리 CLI. XML 티켓 파일(.agent-factory/tickets/active/T-NNN.xml)을 SSoT로 사용

#### 서브커맨드

| 서브커맨드 | 사용법 | 설명 |
|-----------|-------|------|
| `create` | `flow-kanban create "<title>" [--command <cmd>] [--status <todo\|open>]` | 새 티켓 생성 (.agent-factory/tickets/todo\|open/T-NNN.xml). 기본 상태: open |
| `move` | `flow-kanban move <ticket> <target> [--force]` | 티켓을 지정 컬럼으로 이동 |
| `done` | `flow-kanban done <ticket>` | 티켓을 Done으로 이동 + .agent-factory/tickets/done/으로 파일 이동 |
...
flow-kanban update-prompt T-169 \
  ...
  --target ".agent-factory/docs/cli-reference.md 갱신"

flow-kanban update-result T-169 \
  --registrykey 20260325-004729 \
  --plan ".agent-factory/runs/20260325-004729/.../plan.md" \
  --report ".agent-factory/runs/20260325-004729/.../report.md" \
  --workdir ".agent-factory/runs/20260325-004729"
