"""WR-495 Phase 1 Regression Test — production-line workflow backend infrastructure.
...
class TestProductionLineSession(unittest.TestCase):
    ...
    def test_dataclass_defaults(self):
        ...
        channel = ProductionLineSSEChannel(session_id='wf-WR-001-abc')
        s = ProductionLineSession(
            session_id='wf-WR-001-abc',
            work_request='WR-001',
            command='implement',
...
class TestProductionLineSessionRegistry(unittest.TestCase):
    ...
    def test_create_and_get(self):
        s = self.reg.create('wf-WR-001-a', 'WR-001', 'implement', '/tmp/w1')
        self.assertEqual(s.session_id, 'wf-WR-001-a')
        self.assertEqual(self.reg.get('wf-WR-001-a'), s)
        self.assertIsNone(self.reg.get('missing'))

    def test_create_idempotent(self):
        s1 = self.reg.create('wf-WR-002-a', 'WR-002', 'implement', '/tmp/w2')
        s2 = self.reg.create('wf-WR-002-a', 'WR-002', 'implement', '/tmp/w2')
        self.assertIs(s1, s2)

    def test_get_by_work_request(self):
        self.reg.create('wf-WR-003-a', 'WR-003', 'research', '/tmp/w3')
        s = self.reg.get_by_work_request('WR-003')
        self.assertIsNotNone(s)
        self.assertEqual(s.work_request, 'WR-003')

    def test_list_all(self):
        self.reg.create('wf-WR-004-a', 'WR-004', 'implement', '/tmp/w4')
        self.reg.create('wf-WR-005-a', 'WR-005', 'review', '/tmp/w5')
        sessions = self.reg.list_all()
        self.assertEqual(len(sessions), 2)
        keys = {s['session_id'] for s in sessions}
        self.assertEqual(keys, {'wf-WR-004-a', 'wf-WR-005-a'})

    def test_update_step_status_mapping(self):
        self.reg.create('wf-WR-006-a', 'WR-006', 'implement', '/tmp/w6')
        s = self.reg.update_step('wf-WR-006-a', 'PLAN')
        self.assertEqual(s.current_step, 'PLAN')
        self.assertEqual(s.status, 'running')

        s = self.reg.update_step('wf-WR-006-a', 'DONE')
        self.assertEqual(s.status, 'completed')

        # FAILED mapping — to new session
        self.reg.create('wf-WR-007-a', 'WR-007', 'implement', '/tmp/w7')
        s = self.reg.update_step('wf-WR-007-a', 'FAILED')
        self.assertEqual(s.status, 'failed')

    def test_update_step_with_phase(self):
        self.reg.create('wf-WR-008-a', 'WR-008', 'implement', '/tmp/w8')
        s = self.reg.update_step('wf-WR-008-a', 'WORK', phase='P1')
        self.assertEqual(s.current_step, 'WORK')
        self.assertEqual(s.current_phase, 'P1')

    def test_set_status(self):
        self.reg.create('wf-WR-009-a', 'WR-009', 'implement', '/tmp/w9')
        s = self.reg.set_status('wf-WR-009-a', 'completed')
        self.assertEqual(s.status, 'completed')

    def test_add_artifact(self):
        self.reg.create('wf-WR-010-a', 'WR-010', 'implement', '/tmp/w10')
        s = self.reg.add_artifact('wf-WR-010-a', 'plan.md', size=1024)
        self.assertIn('plan.md', s.artifacts)
        self.assertEqual(s.artifacts['plan.md']['size'], 1024)

    def test_remove_vs_purge(self):
        self.reg.create('wf-WR-011-a', 'WR-011', 'implement', '/tmp/w11')
        fpath = os.path.join(self.tmpdir, 'wf-WR-011-a.jsonl')
        self.assertTrue(os.path.exists(fpath))
        # remove → disk preservation
        self.assertTrue(self.reg.remove('wf-WR-011-a'))
        self.assertTrue(os.path.exists(fpath))
        # purge → delete disk
        self.reg.create('wf-WR-012-a', 'WR-012', 'implement', '/tmp/w12')
        fpath2 = os.path.join(self.tmpdir, 'wf-WR-012-a.jsonl')
        self.assertTrue(self.reg.purge('wf-WR-012-a'))
        self.assertFalse(os.path.exists(fpath2))

    def test_persist_meta_first_line(self):
        self.reg.create('wf-WR-013-a', 'WR-013', 'implement', '/tmp/w13', worktree_path='/tmp/wt13')
        fpath = os.path.join(self.tmpdir, 'wf-WR-013-a.jsonl')
        with open(fpath) as f:
            first = json.loads(f.readline())
        meta = first['_meta']
        self.assertEqual(meta['session_id'], 'wf-WR-013-a')
        self.assertEqual(meta['work_request'], 'WR-013')
    ...
    def test_load_from_disk(self):
        from board.server.sessions.production_line_session import ProductionLineSessionRegistry
        self.reg.create('wf-WR-014-a', 'WR-014', 'implement', '/tmp/w14')
        self.reg.create('wf-WR-015-a', 'WR-015', 'research', '/tmp/w15')

        reg2 = ProductionLineSessionRegistry(persist_dir=self.tmpdir)
        loaded = reg2.load_from_disk()
        self.assertEqual(loaded, 2)
        s = reg2.get('wf-WR-014-a')
        self.assertEqual(s.work_request, 'WR-014')
        self.assertEqual(s.status, 'completed')
    ...
    def test_is_fake_session_id_helper(self):
        ...
        self.assertTrue(is_fake_session_id('wf-WR-495-test-p2'))
        self.assertTrue(is_fake_session_id('wf-WR-495-p3-extras-test-749148'))
        self.assertTrue(is_fake_session_id('wf-smoke-001'))
        self.assertTrue(is_fake_session_id('wf-fake-X'))
        self.assertTrue(is_fake_session_id('wf-WR-001-MOCK-x'))  # Case irrelevant
        # Normal production session_id (uuid format) passed
        self.assertFalse(is_fake_session_id('wf-WR-495-123e4567-e89b-12d3-a456-426614174000'))
        self.assertFalse(is_fake_session_id('wf-WR-001-a'))

    def test_create_rejects_fake_session_id(self):
        # ValueError occurs for all 4 fake patterns + does not persist
        for sid in ('wf-WR-001-test', 'wf-smoke-002', 'wf-fake-003', 'wf-mock-004'):
            with self.assertRaises(ValueError) as ctx:
                self.reg.create(sid, 'WR-001', 'implement', '/tmp/w1')
    ...
    def test_create_allows_production_session_id(self):
        ...
        s = self.reg.create(
            'wf-WR-001-123e4567-e89b-12d3-a456-426614174000',
            'WR-001', 'implement', '/tmp/w1'
        )
        self.assertIsNotNone(s)
        self.assertEqual(s.work_request, 'WR-001')
...
class TestProductionLineSSEChannel(unittest.TestCase):
    ...
    def test_broadcast_to_client(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-100-a', persist_path=self.persist_path)
        client = _FakeWFile()
        ch.add(client)
    ...
    def test_persist_writes_jsonl(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-101-a', persist_path=self.persist_path)
        ch.broadcast('workflow_step', {'step': 'PLAN'})
    ...
    def test_emit_step_payload(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-102-a')
        client = _FakeWFile()
        ch.add(client)
    ...
    def test_emit_stdout_payload(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-103-a')
        client = _FakeWFile()
        ch.add(client)
    ...
    def test_emit_phase_payload(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-104-a')
        client = _FakeWFile()
        ch.add(client)
    ...
    def test_emit_finish_payload(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-105-a')
        client = _FakeWFile()
        ch.add(client)
    ...
    def test_client_remove(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-106-a')
        client = _FakeWFile()
        ch.add(client)
        self.assertEqual(ch.client_count(), 1)
        ch.remove(client)
    ...
    def test_dead_client_pruned(self):
        from board.server.channels.production_line_sse_channel import ProductionLineSSEChannel
        ch = ProductionLineSSEChannel(session_id='wf-WR-107-a')
...
class TestProductionLineWorkflowPathRegex(unittest.TestCase):
    ...
    def test_session_path_re(self):
        ...
        m = _SESSION_PATH_RE.match('/api/v2/sessions/wf-WR-100-a')
        self.assertIsNotNone(m)
        self.assertEqual(m.group('session_id'), 'wf-WR-100-a')
        self.assertIsNone(m.group('sub'))

        m = _SESSION_PATH_RE.match('/api/v2/sessions/wf-WR-100-a/events')
        self.assertEqual(m.group('sub'), 'events')

        m = _SESSION_PATH_RE.match('/api/v2/sessions/wf-WR-100-a/step')
        self.assertEqual(m.group('sub'), 'step')

        m = _SESSION_PATH_RE.match('/api/v2/sessions/wf-WR-100-a/artifacts/plan.md')
        self.assertEqual(m.group('sub'), 'artifacts/plan.md')

        m = _SESSION_PATH_RE.match('/api/v2/sessions/wf-WR-100-a/artifacts/work/P1.md')
