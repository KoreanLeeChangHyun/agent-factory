...
def test_workflow_context_paths(tmp_path: Path) -> None:
    ...
    # WR-504 cutover — plan md path / report md path new path
...
def test_context_io_roundtrip(tmp_path: Path) -> None:
    ctx = _make_ctx(tmp_path)
    ctx.feature_branch = "feat/WR-489"
    ctx.worktree_path = Path("/tmp/wt/feat-WR-489")
    ctx.title = "Sample WorkRequest Title"
    ctx.session_ids["wf-WR489-PLAN"] = "abc-uuid"
    write_context(ctx)
    payload = read_context(ctx)
    assert payload["schema_version"] == 1
    assert payload["work_request_no"] == "WR-489"
    assert payload["engine_version"] == "production_line"
    assert payload["feature_branch"] == "feat/WR-489"
    assert payload["worktree_path"] == "/tmp/wt/feat-WR-489"
    assert payload["title"] == "Sample WorkRequest Title"
    assert payload["session_ids"]["wf-WR489-PLAN"] == "abc-uuid"
...
# -------- WR-503 new issue path helpers --------


def test_work_phase_dir(tmp_path: Path) -> None:
    """WR-503 — work phase dir(phase id) → work/<phase>/ directory path."""
    ctx = _make_ctx(tmp_path)
...
def test_work_phase_w_md(tmp_path: Path) -> None:
    """WR-503 — work_phase_w_md(phase_id, worker_idx) → work/<phase>/W<n>.md."""
    ctx = _make_ctx(tmp_path)
...
def test_work_phase_md_resolved_flat_fallback(tmp_path: Path) -> None:
    """WR-503 — nested midfield + flat presence → flat fallback."""
    ctx = _make_ctx(tmp_path)
    flat = ctx.work_dir_phase_md("P1")
    flat.parent.mkdir(parents=True, exist_ok=True)
    flat.write_text("flat", encoding="utf-8")
...
def test_work_phase_md_resolved_neither_returns_nested_default(tmp_path: Path) -> None:
    """WR-503 — both midfields → nested default path return (available as write-target)."""
    ctx = _make_ctx(tmp_path)
    resolved = ctx.work_phase_md_resolved("P1")
...
def test_validate_dir(tmp_path: Path) -> None:
    """WR-503 — validate/ directory path."""
    ctx = _make_ctx(tmp_path)
    assert ctx.validate_dir() == tmp_path / "validate"


def test_validate_nested_paths(tmp_path: Path) -> None:
    """WR-503 — validate/report.md / validate/rules.json / validate/code.json."""
    ctx = _make_ctx(tmp_path)
...
def test_flat_paths_preserved_for_backward_compat(tmp_path: Path) -> None:
    """WR-503 — preserving the old flat path helpers (`validate-report.md` etc.)."""
    ctx = _make_ctx(tmp_path)
    assert ctx.validate_report_md_path() == tmp_path / "validate-report.md"
...
def test_metadata_json_path(tmp_path: Path) -> None:
    """WR-503 — metadata.json path."""
    ctx = _make_ctx(tmp_path)
    assert ctx.metadata_json_path() == tmp_path / "metadata.json"


# -------- WR-503 write_metadata / read_metadata --------


def test_write_metadata_basic(tmp_path: Path) -> None:
    """WR-503 — metadata.json old 4 file (.context.json + status.json + summary.txt + failure.md) Absorption."""
    ctx = _make_ctx(tmp_path)
    ctx.feature_branch = "feat/WR-503"
    ctx.title = "WR-503"
    ctx.session_ids["wf-WR503-PLAN"] = "uuid-1"
    ...
    assert payload["schema_version"] == 1
    assert payload["work_request_no"] == "WR-489"
    assert payload["feature_branch"] == "feat/WR-503"
    assert payload["title"] == "WR-503"
    assert payload["session_ids"]["wf-WR503-PLAN"] == "uuid-1"
...
def test_write_metadata_failure(tmp_path: Path) -> None:
    """WR-503 — failure reason sanctuary failure field reason + ts."""
    ctx = _make_ctx(tmp_path)
...
def test_read_metadata_missing(tmp_path: Path) -> None:
    """WR-503 — metadata.json returns `{}` in the middle."""
    ctx = _make_ctx(tmp_path)
    assert read_metadata(ctx) == {}


def test_read_metadata_present(tmp_path: Path) -> None:
    """WR-503 — metadata.json returns dict when present."""
    ctx = _make_ctx(tmp_path)
...
# -------- WR-506 P1 / P4 — max_parallel + fail_policy --------


def test_max_parallel_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P1 — env Unconfigured + .settings Unspecified → default 4."""
...
def test_max_parallel_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P1 — V2 MAX PARALLEL env priority override."""
...
def test_max_parallel_settings_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P1 — .settings prior to default when env is set."""
    monkeypatch.delenv("V2_MAX_PARALLEL", raising=False)
    from engine.apps.production_line import _common
...
def test_max_parallel_env_beats_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P1 — env > .settings priority."""
...
def test_max_parallel_bogus_input_fallback(
    ...
    """WR-506 P1 — Drinking / 0 / Non-numeric / Hydrophobic → default 4."""
...
def test_fail_policy_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P4 — env aesthetics → 'fail fast'."""
...
def test_fail_policy_env_fail_tolerant(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P4 — V2_FAIL_POLICY=fail_tolerant override."""
...
def test_fail_policy_invalid_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    """WR-506 P4 — Unknown value → default 'fail fast'."""
