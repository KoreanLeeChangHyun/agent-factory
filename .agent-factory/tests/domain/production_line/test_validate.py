"""test validate.py —  validate.py 12-valid rule base evaluation unit test.

Price:
  (R-EXIST-1~4 / R-METRIC-2~3 / R-GUARD-1~3 / R-PATH-1 / R-FSM-1 / R-WT-1)
  - Verdict: PASS / WARN / FAIL
  - hard-fail rules (R-EXIST-1 / R-METRIC-2 / R-WT-1) 1+ violations → FAIL
  - Research / review command SKIP branch
  - validate-rules.json output + schema
"""
...
def _setup_passing_artifacts(ctx: WorkflowContext) -> None:
    """All rules PASS scenarios — verdict=PASS expectations."""
    # R-EXIST-1: report.html (T-504 cutover)
    ctx.report_md_path().write_text(
        "<html><body>This cycle progresses according to plan.md's decision.</body></html>\\n" * 3,
        encoding="utf-8",
    )
    # R-EXIST-2: plan/plan.md (T-504 cutover — nested path)
...
def test_hard_fail_rules_canon() -> None:
    # T-503 — hard-fail rules (R-EXIST-1 + R-METRIC-2 + R-WT-1 + R-CODE-1).
    assert HARD_FAIL_RULES == ("R-EXIST-1", "R-METRIC-2", "R-WT-1", "R-CODE-1")


def test_all_pass(tmp_path: Path) -> None:
    # Stage 3-D Fixation: command=implement scene feature branch required.
    # In the test, configure PASS scenarios with command=research to bypass worktree branch.
...
def test_r_metric_3_tool_deny_violation(tmp_path: Path) -> None:
    ctx = _make_ctx(tmp_path)
    _setup_passing_artifacts(ctx)
    # tool.deny 1 → R-METRIC-3 Violation (hard-fail denim → WARN)
...
def test_r_path_1_report_missing_token(tmp_path: Path) -> None:
    ctx = _make_ctx(tmp_path)
    _setup_passing_artifacts(ctx)
    # Remove plan.md token from report.html
...
def test_r_guard_2_no_feature_branch_skip(tmp_path: Path) -> None:
    """ctx.feature branch=None City SKIP (worktree-less)."""
    ctx = _make_ctx(tmp_path)
    _setup_passing_artifacts(ctx)
    assert ctx.feature_branch is None
    report = evaluate_12_rules(ctx)
...
def test_verdict_warn_with_one_violation(tmp_path: Path) -> None:
    """1 Violation + hard-fail 0 → WARN. Stage 3-D Formulation: Configure PASS scenarios with research base."""
    ctx = _make_ctx(tmp_path, command="research")
    _setup_passing_artifacts(ctx)
    # R-METRIC-3 (tool.deny)
    with ctx.metrics_jsonl_path().open("a", encoding="utf-8") as fh:
        fh.write('{"event":"tool.deny","tool":"Bash"}\n')
    report = evaluate_12_rules(ctx)
...
def test_save_verdict_report_schema(tmp_path: Path) -> None:
    # Stage 3-D Formulation: Pass schema verification with research base
    ctx = _make_ctx(tmp_path, command="research")
    _setup_passing_artifacts(ctx)
    report = evaluate_12_rules(ctx)
    ...
    # T-503 — 14 rules all exist (R-CODE-1/2).
    expected = [
...
def test_save_verdict_report_writes_nested_mirror(tmp_path: Path) -> None:
    """T-503 wire-up — validate/rules.json (nested)"""
    ctx = _make_ctx(tmp_path, command="research")
    _setup_passing_artifacts(ctx)
    report = evaluate_12_rules(ctx)
...
# -------- T-503 R-CODE Lloyd Test --------


def _write_code_json(ctx: WorkflowContext, payload: dict) -> None:
    """validate/code.json fixture writer — P4 ` verify code.read code json`"""
...
def test_r_code_1_skip_no_code_json(tmp_path: Path) -> None:
    """R-CODE-1: validate/code.json Untitled → SKIP (driver  verify code call regression)."""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
...
def test_r_code_1_skip_command_skip(tmp_path: Path) -> None:
    """R-CODE-1: command skip=True → SKIP in code.json"""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    _write_code_json(
        ctx,
...
def test_r_code_1_skip_pytest_missing(tmp_path: Path) -> None:
    """R-CODE-1: pytest items in code.json → SKIP."""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    _write_code_json(
        ctx,
...
def test_r_code_1_skip_pytest_status_skip(tmp_path: Path) -> None:
    """R-CODE-1: pytest status=skip (documentation/configuration) → SKIP."""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    _write_code_json(
        ctx,
...
def test_r_code_1_fail_hard_fail(tmp_path: Path) -> None:
    """R-CODE-1: pytest status=fail → hard-fail (verdict FAIL Force)."""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    # If you want to pass another rule in the implementation scenario, you need feature branch + git matching.
    # This test is valid for R-CODE-1 hard-fail only — has hard fail() True.
    _write_code_json(
        ctx,
        {
            "schema_version": 1,
...
def test_r_code_1_research_skip(tmp_path: Path) -> None:
    """R-CODE-1: command=research → SKIP (worktree-less, code change x)."""
    ctx = _make_ctx(tmp_path, command="research")
    _setup_passing_artifacts(ctx)
    report = evaluate_rules(ctx)
    rule = next(r for r in report.rules if r.rule_id == "R-CODE-1")
...
def test_r_code_2_skip_no_ruff(tmp_path: Path) -> None:
    """R-CODE-2: ruff items in code.json → SKIP."""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    _write_code_json(
        ctx,
...
def test_r_code_2_pass_zero_diagnostics(tmp_path: Path) -> None:
    """R-CODE-2: ruff status=fail but counts.diagnostics=0 → PASS (graceful)."""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    _write_code_json(
        ctx,
...
def test_r_code_2_advisory_fail_no_hard_fail(tmp_path: Path) -> None:
    """R-CODE-2: ruff diagnostics > 0 → advisory FAIL"""
    ctx = _make_ctx(tmp_path, command="implement")
    _setup_passing_artifacts(ctx)
    _write_code_json(
        ctx,
    ...
    # R-CODE-2 is hard-fail but not — the sole violation is WARN or FAIL (if other rules are merged)
    assert "R-CODE-2" not in HARD_FAIL_RULES


def test_evaluate_rules_14_rules_total(tmp_path: Path) -> None:
    """T-503 — Evaluate rules returns all 14 rules (research baseline)."""
    ctx = _make_ctx(tmp_path, command="research")
    _setup_passing_artifacts(ctx)
    report = evaluate_rules(ctx)
...
def test_evaluate_12_rules_alias_returns_same(tmp_path: Path) -> None:
    """T-503 — the same result as the old evaluate 12 rules alias is evaluate rules (backward compat)."""
    ctx = _make_ctx(tmp_path, command="research")
    _setup_passing_artifacts(ctx)
