...
`.agent-factory/.settings` 또는 env `V2_RETRY_<STEP>` 으로 step 별 override 가능.

### 3.4.1 병렬 spawn 정책 (T-506 신규)

WORK Step subprocess 모드의 phase 간 / phase 안 동시 spawn 한계 + 실패 처리.

| 항목 | 기본값 | env override | 의미 |
|------|--------|--------------|------|
| `max_parallel` | 4 | `V2_MAX_PARALLEL` | 같은 topo level 동시 spawn 한계. inner workers pool 은 별도 (phase.workers 그대로). |
| `fail_policy` | `fail_fast` | `V2_FAIL_POLICY` | `fail_fast` (default) / `fail_tolerant`. fail_fast 시 한 phase fail → 같은 level 미시작 future cancel + 다음 level 차단. |
...
- `workers` 기본값 `1`. 2+ 는 phase 안 worker N 동시 spawn (T-506). `parse_plan_json` 이 `workers >= 1` 정합 확인.
- `deliverable` 권장 형식: `work/<id>/W1.md` (T-503 디렉터리 nesting). `work/<id>.md` flat 도 backward compat.
- `deps` 빈 list 허용. 같은 `deps` 를 가진 sibling phase 들은 driver `topo_levels` 로 같은 level 으로 묶여 subprocess 모드에서 동시 spawn 가능 (T-506).
...
### 5.3.1 workers 의미 (T-506 신규)

| 값 | 의미 | 산출물 경로 |
|----|------|-----------|
| `1` (default) | 단일 worker — 기존 경로 (회귀 0) | `work/<id>/W1.md` (또는 flat `work/<id>.md`) |
| `N` (≥ 2) | phase 안 N worker 동시 spawn (subprocess 모드 한정) | `work/<id>/W1.md` ~ `work/<id>/W<N>.md` 모두 작성 의무 |

`workers > 1` 인 phase 는 `spawn_mode=subprocess` 와 함께 사용 권장. in_place 모드는 단일 subprocess 안에서 phase 순차 처리 — workers > 1 의미 없음 (LLM 이 1 subprocess 안에서 N 산출물 작성).

driver 구현: `_spawn_one_phase` 가 `phase.workers > 1` 분기 — `parallel_spawn(worker_indices, ...)` nested pool. 외부 level pool (`max_parallel` 가드) 과 독립.

위험 영역: outer level pool (max=4) × inner workers pool (workers=N) → 최대 동시 4×N. CPU 가 아니라 subprocess I/O bound 라 N×4 가 8~12 까지 안전. 그 이상은 사용자 명시 plan 결정 책임.
...
### 6.5 병렬 spawn 정책 (T-506 신규)

WORK Step subprocess 모드의 phase 간 + phase 안 병렬 spawn 정책. PLAN/VALIDATE/REPORT 는 본 정책 미적용 (단일 spawn).
...
3. 모든 phase 종료 후 다음 level 진입

#### 6.5.2 phase 안 worker 병렬 (workers > 1)

`_spawn_one_phase` 가 `phase.workers > 1` 시 nested `parallel_spawn(worker_indices, ...)` — N session_id 동시 spawn → `work/<id>/W1.md` ~ `work/<id>/W<N>.md` 모두 작성. inner pool 의 max_workers 는 `phase.workers` 그대로 (outer pool 의 `max_parallel` 가드와 독립).

#### 6.5.3 fail_fast 정책

`get_fail_policy() == "fail_fast"` (default) 일 때:
- level 안 한 phase fail → 같은 level 미시작 future cancel (`ThreadPoolExecutor.shutdown(cancel_futures=True)`)
- 같은 level 모든 outcome 집계 후 `any_fail` True → 다음 level 진입 차단

`fail_tolerant` 일 때 — level 안 모든 phase 끝까지 실행 + 다음 level 도 진입 (실패가 level 전이 차단하지 않음).
...
#### 6.5.5 running subprocess kill 미적용 (별 트랙)

`fail_fast` 시 `cancel_futures=True` 는 **시작 안 한 future 만 cancel**. 이미 running 중인 claude -p subprocess 는 끝까지 진행 (강제 kill 안 함). 본 사이클은 "다음 level 차단" 의미. running subprocess 강제 종료는 별 트랙 (T-506 후속).
...
claude -p subprocess 의 `cwd` = `ctx.work_dir` (= `.agent-factory/runs/<key>/`). prompt 가 "산출물 경로 = `plan.md`" 또는 "`work/P1.md`" 식으로 명시. claude -p 가 Write/Edit 도구로 직접 write.
...
driver `init_step` 안에서 `worktree_manager.create_worktree(ticket_no, title, command=...)` 를 호출하고 `command != "implement"` 인 경우 None 반환 → ctx.feature_branch=null + work_dir 은 메인 `.agent-factory/runs/<key>/`. `command == "implement"` 인 경우 ctx.feature_branch + work_dir 은 `<worktree_path>/.agent-factory/runs/<key>/` 안.
...
`/wf -s N` → `.agent-factory/bin/flow-wf submit T-N` 호출. 메인 세션은 trigger 만, 진행은 driver.
