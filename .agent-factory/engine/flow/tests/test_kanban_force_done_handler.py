...
# handlers/ 경로 계산: tests/ → engine/ → .agent-factory/ → board/server/handlers/
_WORKTREE_ROOT = Path(__file__).resolve().parent.parent.parent.parent.parent
_HANDLERS_DIR = _WORKTREE_ROOT / ".agent-factory" / "board" / "server" / "handlers"
...
class TestOpenDoneForceBranchCallsMoveWithForce(unittest.TestCase):
    ...
    def test_open_done_force_branch_calls_move_with_force(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            open_dir = os.path.join(tmpdir, ".agent-factory", "tickets", "open")
            ...
            bin_dir = os.path.join(tmpdir, ".agent-factory", "bin")
...
class TestDeleteBlockedWhenDerivedTicketOpen(unittest.TestCase):
    ...
    tempfile 로 격리된 .agent-factory/tickets/ 트리를 구성하여
    실제 파일시스템 읽기를 테스트한다.
    """
