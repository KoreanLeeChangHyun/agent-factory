"""M9 structured verification and final verdict model.

This module is driver-owned and deterministic. It separates:
- VERIFY-stage artifact/code/semantic artifacts (`validate/verdict.json`)
- REPORT-stage manifest (`report.json`)
- COMPLETE-stage gate decision (`final-verdict.json`)
"""
...
class Gate:
    ...
    source: str    # artifact | metric | guard | path | fsm | code
...
def _result_payload(name: str, result: VerifyResult, path: Path | None = None) -> dict[str, Any]:
...
def _compute_preliminary_verdict(checks: Iterable[dict[str, Any]]) -> str:
...
def write_verify_verdict(ctx: WorkflowContext) -> Path:
    """Write VERIFY-stage structured verdict data.

    `validate/report.md` remains the semantic LLM evaluation. This file records
    deterministic artifact/code checks and links that semantic artifact without
    asking the LLM to produce a verdict.
    """
    ...
    code_payload = _verify_code.read_code_json(ctx)
    ...
    checks = [
        {**_result_payload("plan_artifacts", plan_result, ctx.plan_dir()), "blocking": False},
        {**_result_payload("work_artifacts", work_result, ctx.work_dir / "work"), "blocking": False},
        {
            **_result_payload(
    ...
    payload = {
        ...
        "preliminary_verdict": _compute_preliminary_verdict(checks),
        "refinement": {
...
def read_verify_verdict(ctx: WorkflowContext) -> dict[str, Any]:
    path = ctx.validate_verdict_json_path()
    if not path.exists():
        return {}
    try:
...
def write_report_manifest(ctx: WorkflowContext) -> Path:
...
def build_final_verdict(ctx: WorkflowContext, verdict_report: Any) -> dict[str, Any]:
    ...
    for rule in rules:
        # dataclass from _validate.RuleResult; duck-typed to avoid import cycle.
...
def save_final_verdict(ctx: WorkflowContext, payload: dict[str, Any]) -> Path:
