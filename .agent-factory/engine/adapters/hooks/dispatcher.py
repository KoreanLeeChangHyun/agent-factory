"""Dispatcher common utilities for Claude Code hook entrypoints.

Provides shared functions for loading .agent-factory/.settings
flags and dispatching hook scripts based on HOOK_* environment variable toggles.

Top-level scripts in ``.agent-factory/hooks`` are stable Claude Code
entrypoints. This module holds their shared implementation so hook adapters
can evolve behind that boundary.
"""
...
def _env_path() -> str:
    ...
    Returns:
        Absolute path to .agent-factory/.settings.
    """
    return os.path.join(_find_project_root(), '.agent-factory', '.settings')


def load_env_flags(prefix: str = 'HOOK_') -> dict[str, bool]:
    """Parse .agent-factory/.settings and return HOOK_* flags as a dict.
...
def scripts_dir(*parts: str) -> str:
    """Return absolute path under .agent-factory/engine/.

    Args:
        *parts: Path components after .agent-factory/engine/.

    Returns:
        Absolute path string.
    """
    root = _find_project_root()
    return os.path.join(root, '.agent-factory', 'engine', *parts)
