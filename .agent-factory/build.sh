...
validate_templates() {
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        ...
        print_info "build.sh 실행 전 .agent-factory/build-assets/templates/ 디렉터리를 확인하세요."
        exit 1
    fi
}
...
PYEOF
    ...
    if echo "$py_output" | grep -q "^ADDED:"; then
        ...
        print_success "settings.json stale 경로 ${fixed_count}건 정합화 (.claude.workflow/ → .agent-factory/)"
    else
...
# --- Step 5: .agent-factory/.settings 생성 ---
generate_claude_settings() {
    print_step "5" ".agent-factory/.settings 생성"
    ...
    if [ -f "$env_file" ]; then
        ...
        print_success ".agent-factory/.settings 생성 완료 (.env에서 복사) ($settings_file)"
        return 0
    fi
    # 둘 다 없으면 템플릿에서 생성
    cp "$TMPL_CLAUDE_ENV" "$settings_file"
    print_success ".agent-factory/.settings 템플릿 생성 완료 ($settings_file)"
}
...
verify_installation() {
    ...
    # (d) .agent-factory/tickets/, .agent-factory/tickets/{todo,open,progress,review,done}/ 디렉터리 존재
    ...
    for dir in ".agent-factory/tickets" ".agent-factory/tickets/todo" ".agent-factory/tickets/open" ".agent-factory/tickets/progress" ".agent-factory/tickets/review" ".agent-factory/tickets/done"; do
        if [ ! -d "$dir" ]; then
            print_error "필수 디렉터리 없음: $dir"; tickets_ok=false; failed=$((failed + 1))
        fi
    done
    [ "$tickets_ok" = true ] && print_success ".agent-factory/tickets/, .agent-factory/tickets/{todo,open,progress,review,done}/ 디렉터리 존재 확인"
    # (e) .agent-factory/.settings 또는 .agent-factory/.env 파일 존재
    if [ -f ".agent-factory/.settings" ]; then
        print_success ".agent-factory/.settings 파일 존재 확인"
    elif [ -f ".agent-factory/.env" ]; then
        print_success ".agent-factory/.env 파일 존재 확인 (폴백)"
    else
        print_error ".agent-factory/.settings 및 .agent-factory/.env 파일이 모두 없습니다"
        failed=$((failed + 1))
    fi
    # (f) .claude/ 디렉터리 존재
